...
public class ConnectDB {
	...
	DBType, String url, String user, String password)

	throws SQLException {
		if ("mysql".equalsIgnoreCase(DBType))
			return getMySqlConn(url, user,

			password);
		if ("oracle".equalsIgnoreCase(DBType))
			return getOracleConn(url, user,

			password);
		if ("sqlserver".equals(DBType)) {
			return getSqlServerConn(url,
	...
	private static Connection getMySqlConn(String
	...
	private static Connection getOracleConn(String
	...
	SQLException {
		...
		try {
			...
			("com.microsoft.jdbc.sqlserver.SQLServerDriver");// 加载
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		conn = DriverManager.getConnection

		(SQLSERVER + url, "root", "root");
	...
	public static void main(String[] args) {
		try {
			...
			if (conn == null) {
				...
				("Connection the database is failled !");
			} else {
				System.out.println
